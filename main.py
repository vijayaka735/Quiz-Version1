...
async def ai_create_quiz(client, message: Message):
    ...
    await message.reply(
        ...
        "1. **Your own Gemini key** — set it once with `/gemini <your_key>` "
...
async def read_questions_from_file(file_path: str, user_id: str, remove_words: Optional[List[str]] = None, 
    ...
    try:
        if lower.endswith('.json'):
            ...
            return await _process_pdf_file(file_path, user_id, remove_words)
        else:
...
async def _process_pdf_file(file_path: str, user_id: str, remove_words: Optional[List[str]] = None) -> Tuple[Optional[int], Optional[str]]:
    ...
    try:
        ...
        try:
            return await _process_txt_file(txt_path, user_id, remove_words)
        finally:
            try:
...
async def handle_all_messages(client, message):
    ...
    if user_id in user_quiz_data:
        ...
        if user_data.get("awaiting_name"):
            ...
            await message.reply(f"✅ Quiz name set to: **{quiz_name}**\nNow send questions in the stated format, or upload a `.txt`, `.json` or `.pdf` file, or forward quiz polls. Send /done when finished or /cancel to stop.")
            return
        ...
        if user_data.get("awaiting_negative_marking"):
            ...
            user_quiz_data[user_id]["awaiting_shuffle_questions"] = True
            await message.reply(
                "🔀 **Shuffle Questions?**\n\n"
                "Reply `yes` to randomise the order of questions every time the quiz is started, or `no` to keep them in the original order.\n\n"
                "_Tip: shuffling is great for live group quizzes — different students see questions in different order._"
            )
            return

        if user_data.get("awaiting_shuffle_questions"):
            choice = message.text.strip().lower()
            if choice not in ["yes", "no", "y", "n"]:
                await message.reply("❌ Please reply with `yes` or `no`.")
                return
            user_quiz_data[user_id]["shuffle"] = choice in ("yes", "y")
            del user_quiz_data[user_id]["awaiting_shuffle_questions"]
            user_quiz_data[user_id]["awaiting_shuffle_options"] = True
            await message.reply(
                "🔀 **Shuffle Options?**\n\n"
                "Reply `yes` to randomise the A/B/C/D order of options for each question (the correct answer stays correct, just moved), or `no` to keep options in the original order."
            )
            return

        if user_data.get("awaiting_shuffle_options"):
            choice = message.text.strip().lower()
            if choice not in ["yes", "no", "y", "n"]:
                await message.reply("❌ Please reply with `yes` or `no`.")
                return
            user_quiz_data[user_id]["shuffle_options"] = choice in ("yes", "y")
            del user_quiz_data[user_id]["awaiting_shuffle_options"]
        ...
        if user_data.get("awaiting_type"):
            ...
            # Capture shuffle flags into locals BEFORE the user_quiz_data
            # entry is deleted below — otherwise the success-message string
            # would crash with KeyError on the missing user_id.
            shuffle_q = user_quiz_data[user_id].get("shuffle", False)
            shuffle_o = user_quiz_data[user_id].get("shuffle_options", False)
